...
module.exports = {
    ...
    json: async function (req, res) {
        var everycourses = await Course.find().populate("evaluation");
        return res.json(everycourses);
    },

    populate: async function (req, res) {

        var course = await Course.findOne(req.params.id).populate("student").populate("evaluation");
        if (!isCreated) return res.notFound();
    ...
    update: async function (req, res) {
        ...
        if (req.method == "GET") {
            ...
            var thatCourse = await Course.updateOne(req.params.id).set(req.body)
            if (!thatCourse) return res.notFound();
