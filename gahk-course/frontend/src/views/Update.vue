...
export default {
              ...
              validators: [
                {
                  ...
                  text: "Please correct the age. The first value should be large or equal to the second one.",
                },
              ],
              items: [
                {
  ...
  async mounted() {
    let response = await fetch("http://localhost:1337/course/read/"+ this.$route.params.id);
  ...
  methods: {
    async alertResults(sender) {
      var response = await fetch("http://localhost:1337/course/update/" + this.$route.params.id, {
        method: "POST",
